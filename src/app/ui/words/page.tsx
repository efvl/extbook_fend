...
async function getWords(page = 0, bookId?: string) {
  ...
  // Choose endpoint based on whether we are filtering by book
  const url = bookId 
...
export default async function WordsPage({ 
  ...
  const data = await getWords(page, bookId);
    ...
    let bookTitle = "";
    if (bookId) {
        const bookRes = await fetch(`${process.env.BACKEND_URL}/v1/book/${bookId}`);
...
return (
    <div className="p-8">
        ...
        <div>
            ...
            {bookId && (
            <Link href="/ui/words" className="text-xs text-blue-600 hover:underline">
                ← Clear filter and show all words
            </Link>
            )}
        </div>
        
        <Link href="/ui/words/new" className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 shadow-md transition">
            + Add Word
        </Link>
        </div>
        ...
        <tbody className="divide-y divide-gray-200 bg-white">
            ...
            <tr key={word.id} className="hover:bg-gray-50 transition-all duration-200">
                ...
                {/* Text Content with "Copy to Clipboard" feel */}
                ...
                {/* Position Badge: Shows Pg/Ln/Wd in a compact pill */}
                <Td>
                ...
                <div className="flex justify-end gap-2 items-center">
                    ...
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            <path d="M3 6h18 text-gray-500"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/>
                        </svg>
                    </button>
                    </form>
                </div>
        ...
        {/* Ensure your Pagination component also carries the bookId forward */}
        <Pagination 
            page={data.number} 
